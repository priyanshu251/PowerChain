...
contract DoubleSidedAuction is ERC20 {
    ...
    struct User {
        ...
        uint[] BQty; // quantity of energy bought from market
        uint[][] SQty; // quantity of energy sold which it suppose to produce
        uint[][] SPrice; // price of each quantity sold
        uint[] sellcount; // used to count number of participants it has satisfied with energy produced
        uint userDebt;
        uint RelayState;
        uint UtilityDebt;
    }
    ...
    modifier OnlyAppNode(address _addrr) {
        require(AppToSmartmeter[_addrr] == userIDtoAddress[userAddresstoID[AppToSmartmeter[_addrr]]], "Not Application Node");
        _;
    }

    modifier OnlyUtility(address _UtilityAddr) {
        require(_UtilityAddr == Utility, "Not Generator Application Node");
        _;
    }

    modifier AskTkrequirements(uint _qty) {
        require(balanceOf[AppToSmartmeter[msg.sender]] >= _qty + MinBal, "Not Enough balance");
        _;
    }

    uint[] public Sp; // Sell price
    uint[] public Sq; // Sell quantity
    address[] public SAddr; // Seller address

    uint[] public Bp; // Buy price
    uint[] public Bq; // Buy quantity
    address[] public BAddr; // Buyer address
    uint public r1; // This is used to count how many transactions were done after each matching

    uint public MinBal = 500; // minimum balance a user needs to leave in the account while selling token

    MergedAskBid[] public ArrayWonBid;

    mapping(uint => uint) public RefId;
    mapping(uint => uint) internal RefIndex;

    mapping(uint => address) public userIDtoAddress;
    mapping(address => uint) public userAddresstoID;
    ...
    function viewSellTokenOffers() public view returns (uint[] memory Sell_Price, uint[] memory Sell_Qty) {
        return (Sp, Sq);
    }

    function viewBuyTokenOffers() public view returns (uint[] memory Buy_Price, uint[] memory Buy_Qty) {
        return (Bp, Bq);
    }

    function _sortSellPrices(uint[] memory Arr_, uint[] memory qty_, address[] memory _sAddr) internal pure returns (uint[] memory, uint[] memory, address[] memory) {
    ...
    function _sortBuyPrices(uint[] memory Arr_, uint[] memory qty_, address[] memory _bAddr) internal pure returns (uint[] memory, uint[] memory, address[] memory) {
        uint256 l = Arr_.length;
    ...
    function clearTokenOffers() public onlyAlarm() {
    ...
    function _assignMatchedPrices(uint _Avp, uint _Avq) internal returns (uint) {
        ...
        ref = uint(keccak256(abi.encodePacked(SAddr[0], BAddr[0], block.timestamp, r1)));
        ...
        if (balanceOf[AppToSmartmeter[SAddr[0]]] >= _Avq) {
            ...
            ref = ref % RefModulus;
            ArrayWonBid.push(MergedAskBid(_Avp, _Avq, SAddr[0], BAddr[0], ref, 0));
_id = ArrayWonBid.length - 1;
            RefId[_id] = ref;
            RefIndex[ref] = _id;
            return 2;
        } else {
