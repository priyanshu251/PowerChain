...
contract ERC20 {
    ...
    mapping (address => address) public AppToSmartmeter;
    ...
    modifier onlyAlarm() {
        require(msg.sender == ExchangeAddr, "Sender not Authorized.");
        _;
    }
    ...
    // Function to change token symbol and name
    function ChangeTokenSymbName(string memory tokenName, string memory tokenSymbol) public onlyOwner {  
        name = tokenName;   
        symbol = tokenSymbol; 
    }
    ...
    function _transfer(address _from, address _to, uint256 _value) internal returns (bool) {
        ...
        if (balanceOf[_from] >= _value) {
            ...
            // Assert is not needed in newer Solidity versions for overflow/underflow
            require(balanceOf[_from] + balanceOf[_to] == previousBalances, "Balance mismatch");
            return true;
        } else {
            return false;
        }
    ...
    function UpdateAllowance(address _spender, uint256 _value) public returns (bool success) {
        allowance[msg.sender][_spender] = _value;
        emit Approval(msg.sender, _spender, _value);
        return true;
    }

    // Add tokens to the owner's account
    ...
    // This function is used by the user's app to check the balance in the corresponding smart meter address
